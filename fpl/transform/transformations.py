...
def calculate_diff(df_grouped: pd.core.frame.DataFrame, columns: list):
    ...
    df_g = df_grouped.groupby(by=["code", "season", "gameweek"], as_index=False).last()
    df_g.loc[df_g.gameweek != 0, columns] = (
        df_g[df_g.gameweek != 0][columns.copy() + ["code", "season"]]
        .groupby(by=["code", "season"])
        .diff()
        .fillna(df_g)
    )
    return df_g
...
def join_elements_and_team(elements_df, teams_df):
    ...
    """
    elements_df = elements_df.groupby(by=["code", "season", "gameweek"], as_index=False).last()
    teams_df = teams_df.groupby(by=["code", "season", "gameweek"], as_index=False).last()
    return elements_df.join(
        teams_df.set_index(["code", "season", "gameweek"]),
        on=["team_code", "season", "gameweek"],
        rsuffix="_team",
    )
